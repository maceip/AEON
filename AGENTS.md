...
- **`docs/TODAY.md`** — the day's prioritized implementation plan. Phases are sequential.
...
|------|------------------|
| `runtime/` | C++ emulator: libriscv integration, syscall handlers, VFS, ELF loader, checkpoint |
| `runtime/checkpoint.hpp` | Binary machine state serialization (CPU, memory, scheduler) |
| `runtime/library_vectorheart.js` | VectorHeart JSPI bridge — async I/O between guest syscalls and browser APIs |
...
| `friscy-bundle/` | Browser deployment: Worker, JIT manager, network bridge, overlay.js, checkpoint |
| `friscy-bundle/overlay.js` | OPFS overlay persistence: delta, sessions, tar utils, Storage Buckets |
| `src/` | React frontend: TypeScript + Vite |
| `src/lib/FriscyMachine.ts` | Machine lifecycle — boot, Web Locks, polling, checkpoint, persistence |
| `src/lib/PackageManager.ts` | OPFS package layer management (install/uninstall/applyLayers) |
| `src/workers/emulator.worker.ts` | Web Worker (emulator loop, checkpoint restore, throttle) |
| `scripts/` | Build & utility scripts (serve-dist, claude-cli, sign-bundle) |
...
| `docs/` | Documentation, plans, research, session logs, assets |
...
4. SharedArrayBuffer is the primary runtime channel. `postMessage` is used for setup, VFS export, checkpoint, and control signals.
...
7. Persistence has ONE write path: Worker → `vfs_export` postMessage → main thread `computeDelta` → `saveOverlay` to OPFS. The Worker never writes to OPFS directly.
8. All browser APIs must be feature-detected. Graceful degradation is mandatory.
